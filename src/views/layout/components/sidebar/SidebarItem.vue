    ...
    <template v-for="item in menus" v-if="item.children">
      ...
      <router-link v-if="item.children.length===1 && !item.children[0].children"
        ...
        <el-menu-item :index="item.path+'/'+item.children[0].path" :class="{'submenu-title-noDropdown':!isNest}">
          ...
          <svg-icon v-if="!item.children[0].meta.icon&&item.meta.icon" :icon-class="item.meta.icon"></svg-icon> <!-- 用父的ICON -->
      ...
      <el-submenu v-else :index="item.name||item.path" :key="item.name">
        ...
        <template v-for="child in item.children">
          ...
          <sidebar-item :is-nest="true" class="nest-menu" v-if="child.children&&child.children.length>0" :menus="[child]" :key="child.path"></sidebar-item>
